...
CREATE TABLE IF NOT EXISTS code (id int AUTO_INCREMENT,content VARCHAR(255),expiry TIMESTAMP, scope VARCHAR(255),PRIMARY KEY (id), user_id int, FOREIGN KEY (user_id) REFERENCES user(id), code_challenge_method VARCHAR(255), code_challenge VARCHAR(255));
